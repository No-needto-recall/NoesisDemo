...
void FTypeScriptDeclarationGenerator::GenTypeScriptDeclaration(bool InGenStruct, bool InGenEnum)
{
    ...
    for (FAssetData const& AssetData : AssetList)
    {
        ...
        if (BlueprintTypeDeclInfoPtr && BlueprintTypeDeclInfoPtr->Changed)
        {
            ...
            if (auto Blueprint = Cast<UBlueprint>(Asset))
            {
                ...
                {
                    UE_LOG(Puerts, Warning, TEXT("invalid blueprint: %s"), *AssetData.PackageName.ToString());
                }
            }
    ...
    UE_LOG(Puerts, Warning, TEXT("[PuerTS] Saving ue.d.ts to: %s"), *UEDeclarationFilePath);
    FFileHelper::SaveStringToFile(ToString(), *UEDeclarationFilePath, FFileHelper::EEncodingOptions::ForceUTF8WithoutBOM);
    UE_LOG(Puerts, Warning, TEXT("[PuerTS] ue.d.ts saved successfully"));
    ...
    UE_LOG(Puerts, Warning, TEXT("[PuerTS] Saving ue_bp.d.ts to: %s"), *BPDeclarationFilePath);
    FFileHelper::SaveStringToFile(ToString(), *BPDeclarationFilePath, FFileHelper::EEncodingOptions::ForceUTF8WithoutBOM);
    UE_LOG(Puerts, Warning, TEXT("[PuerTS] ue_bp.d.ts saved successfully"));
}
...
void FTypeScriptDeclarationGenerator::Gen(UObject* ToGen)
{
    ...
    {
        UE_LOG(Puerts, Warning, TEXT("duplicate name found in ue.d.ts generate: %s"), *ToGen->GetName());
        return;
    }
    Processed.Add(ToGen);
    if (ToGen->IsNative())
...
void FTypeScriptDeclarationGenerator::GenStruct(UStruct* Struct)
{
    ...
    if (const auto UserDefinedStruct = Cast<UUserDefinedStruct>(Struct))
    {
        ...
        {
            UE_LOG(Puerts, Error, TEXT("User Defined Struct %s has error:%s"), *UserDefinedStruct->GetName(),
...
private:
    ...
    {
        UE_LOG(Puerts, Warning, TEXT("[PuerTS] GenUeDts started"));
        
        GenTypeScriptDeclaration(InGenFull, InSearchPath);
        UE_LOG(Puerts, Warning, TEXT("[PuerTS] GenTypeScriptDeclaration completed"));
        ...
        UE_LOG(Puerts, Warning, TEXT("[PuerTS] Code generators executed"));
        ...
        UE_LOG(Puerts, Warning, TEXT("[PuerTS] Generation completed, PATH=%s"), *PackagePath.ToString());
        ...
        UE_LOG(Puerts, Warning, TEXT("[PuerTS] %s"), *DialogMessage);
    }
...
public:
    void StartupModule() override
    {
        ...
        ConsoleCommand = MakeUnique<FAutoConsoleCommand>(TEXT("Puerts.Gen"), TEXT("Execute GenDTS action"),
            FConsoleCommandWithArgsDelegate::CreateLambda(
                ...
                {
                    UE_LOG(Puerts, Warning, TEXT("[PuerTS] Puerts.Gen command started with args: %s"), *FString::Join(Args, TEXT(", ")));
                    ...
                    UE_LOG(Puerts, Warning, TEXT("[PuerTS] Calling GenUeDts with GenFull=%d, SearchPath=%s"), GenFull, *SearchPath.ToString());
    ...
    {
        UE_LOG(Puerts, Warning, TEXT("[PuerTS] Step 1: Creating TypeScriptDeclarationGenerator"));
        FTypeScriptDeclarationGenerator TypeScriptDeclarationGenerator;
        
        UE_LOG(Puerts, Warning, TEXT("[PuerTS] Step 2: RestoreBlueprintTypeDeclInfos"));
        TypeScriptDeclarationGenerator.RestoreBlueprintTypeDeclInfos(InGenFull);
        
        UE_LOG(Puerts, Warning, TEXT("[PuerTS] Step 3: LoadAllWidgetBlueprint with path: %s"), *InSearchPath.ToString());
        TypeScriptDeclarationGenerator.LoadAllWidgetBlueprint(InSearchPath, InGenFull);
        
        UE_LOG(Puerts, Warning, TEXT("[PuerTS] Step 4: GenTypeScriptDeclaration"));
        TypeScriptDeclarationGenerator.GenTypeScriptDeclaration(true, true);
        
        UE_LOG(Puerts, Warning, TEXT("[PuerTS] Step 5: Completed all steps"));
    }
