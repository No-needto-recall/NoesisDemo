...
exports.createNoesisProxy = createNoesisProxy;
 ...
 */
function createNoesisProxy(target, propertyNames, enableLogging = false) {
    // 判空处理
    if (!target) {
    ...
    if (propertyNames && propertyNames.length > 0) {
        ...
        while (proto && proto !== UE.Object.prototype) {
            if (proto.__noesisNotifyProps) {
                proto.__noesisNotifyProps.forEach((p) => notifyProps.add(p));
            }
        ...
        if (notifyProps.size === 0) {
            ...
            for (const key in typedTarget) {
                if (typedTarget.hasOwnProperty(key) && typeof typedTarget[key] !== 'function') {
                    notifyProps.add(key);
                }
            }
        }
            ...
            if (value && typeof value === 'object' && typeof prop === 'string') {
                ...
                if (notifyProps.has(prop)) {
                    ...
                    if (enableLogging) {
                        ...
                        const availableMethods = methods.filter(m => m in value && typeof value[m] === 'function');
 ...
 */
function createTArrayProxy(owner, propName, tarray, enableLogging = false) {
 ...
 */
function createTMapProxy(owner, propName, tmap, enableLogging = false) {
...
//# sourceMappingURL=NoesisProxy.js.map